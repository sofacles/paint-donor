...
describe('GiveAwayPaint form', () => {
  ...
  it('POSTs a paint unit when you submit the form', async () => {
    ...
    const postUrl = decodeURI(axios.post.mock.calls[0][0] + '').replace(
      '%23',
      '#'
    ); //%23 is #, is not decoded by decodeURI
    ...
    stateKeys.forEach((k) => {
      ...
      if (postUrl.indexOf(someState[k]) === -1) {
        console.log(
          `******** problem with k: ${k}  + someState[k]: ${someState[k]}`
        );
      }
  ...
  describe('Error messages when a single field is missing', () => {
    it('shows missing name Errors', async () => {
      ...
      fireEvent.change(
        getByLabelText('Brand:'),
        makeEventArgs('brand', 'Behr')
      );
      fireEvent.change(
        getByLabelText('Quantity:'),
        makeEventArgs('quantity', 'less than a gallon')
      );
      fireEvent.change(
        getByLabelText('Email:'),
        makeEventArgs('email', 'someString')
      );
      fireEvent.change(
        getByLabelText('Confirm Email:'),
        makeEventArgs('confirmEmail', 'someString')
      );
      fireEvent.change(
        getByLabelText('Zip Code:'),
        makeEventArgs('zipCode', '99999')
      );
    ...
    it('shows missing brand Errors', async () => {
      ...
      fireEvent.change(
        getByLabelText('Color Name:'),
        makeEventArgs('name', 'savage')
      );
      fireEvent.change(
        getByLabelText('Quantity:'),
        makeEventArgs('quantity', 'less than a gallon')
